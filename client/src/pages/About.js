import React from 'react';

export default function About() {
    
    return (
        <div className='bg-ocean-trash-image w-full h-full bg-cover bg-center flex items-center px-4'>
            <div className='uppercase text-center text-white font-extrabold w-1/2 justify-center'>
                <h1 className='text-8xl'> Tailwind CSS</h1>
                <h3>Join your local community environment group</h3>
                <div className=" gap-2">
            </div>
            </div>
                <div className='text-center justify-end content-end w-1/2'>
            <div className='flex flex-wrap'>
                <h3 className="uppercase text-black font-extrabold text-2xl bg-white/80">“We are the first generation to feel the effect of climate change and the last generation who can do something about it.” - 
                <span className="italic">Barack Obama</span>
                </h3>
            </div>
        </div>
  
    </div>  
    )
}